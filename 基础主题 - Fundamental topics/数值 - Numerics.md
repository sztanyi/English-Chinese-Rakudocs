原文：https://docs.perl6.org/language/numerics
...
与某些语言不同，当两个操作数都是 [Int](https://docs.perl6.org/type/Int) 类型时，使用 [`/` 操作符](https://docs.perl6.org/routine/$SOLIDUS)执行的除法将产生一个分数，而不执行任何取整。

Unlike some languages, division performed using [`/` operator](https://docs.perl6.org/routine/$SOLIDUS) when both operands are of [Int](https://docs.perl6.org/type/Int) type, would produce a fractional number, without any rounding performed.
...
相除生成的类型是 [Rat](https://docs.perl6.org/type/Rat) 或 [Num](https://docs.perl6.org/type/Num) 类型。如果分数的分母小于 64 位，则生成 [Rat](https://docs.perl6.org/type/Rat)，否则生成 [Num](https://docs.perl6.org/type/Num) 类型。
...
[Num](https://docs.perl6.org/type/Num) 类型提供 [双精度浮点](https://en.wikipedia.org/wiki/Double-precision_floating-point_format)十进制数，在其他语言中有时称为 "doubles"。
...
区分大小写的单词 [Inf](https://docs.perl6.org/type/Num#Inf) 和 [NaN](https://docs.perl6.org/type/Num#NaN) 分别表示特殊值无穷大和非数字。可以使用 U+221E 表示无限（`∞`）字符而不是 [Inf](https://docs.perl6.org/type/Num#Inf)：

Case-sensitive words [Inf](https://docs.perl6.org/type/Num#Inf) and [NaN](https://docs.perl6.org/type/Num#NaN) represent the special values infinity and not-a-number respectively. The U+221E INFINITY (`∞`) character can be used instead of [Inf](https://docs.perl6.org/type/Num#Inf):

Perl 6 尽可能遵循 [IEEE 754-2008 浮点算术标准](https://en.wikipedia.org/wiki/IEEE_754)，计划在以后的语言版本中实现更多的一致性。该语言保证为任何给定的 [Num](https://docs.perl6.org/type/Num) 文本选择最接近的可表示数字，并支持负零和[非规格化数字](https://en.wikipedia.org/wiki/Denormal_number)（也称为 "subnormals"）。

Perl 6 follows the [IEEE 754-2008 Standard for Floating-Point Arithmetic](https://en.wikipedia.org/wiki/IEEE_754) as much as possible, with more conformance planned to be implemented in later language versions. The language guarantees the closest representable number is chosen for any given [Num](https://docs.perl6.org/type/Num) literal and does offer support for negative zero and [denormals](https://en.wikipedia.org/wiki/Denormal_number) (also known as "subnormals").

请记住，输出例程（如 [say](https://docs.perl6.org/routine/say) 或 [put](https://docs.perl6.org/routine/put) 不会很难区分 [Numeric](https://docs.perl6.org/type/Numeric) 类型是如何输出的，并且可以选择显示 [Num](https://docs.perl6.org/type/Num) 作为 [Int](https://docs.perl6.org/type/Int) 或 [Rat](https://docs.perl6.org/type/Rat) 编号。要输出更明确的字符串，请使用 [perl](https://docs.perl6.org/routine/perl) 方法：

Keep in mind that output routines like [say](https://docs.perl6.org/routine/say) or [put](https://docs.perl6.org/routine/put) do not try very hard to distinguish between how [Numeric](https://docs.perl6.org/type/Numeric) types are output and may choose to display a [Num](https://docs.perl6.org/type/Num) as an [Int](https://docs.perl6.org/type/Int) or a [Rat](https://docs.perl6.org/type/Rat) number. For a more definitive string to output, use the [perl](https://docs.perl6.org/routine/perl) method:
...
[复平面](https://en.wikipedia.org/wiki/Complex_plane)的[复数](https://docs.perl6.org/type/Complex)型数值。[复数](https://docs.perl6.org/type/Complex) 对象由两个 [Num](https://docs.perl6.org/type/Num) 对象组成，表示复数的[实部](https://docs.perl6.org/routine/re)和[虚部](https://docs.perl6.org/routine/im)部分。

The [Complex](https://docs.perl6.org/type/Complex) type numerics of the [complex plane](https://en.wikipedia.org/wiki/Complex_plane). The [Complex](https://docs.perl6.org/type/Complex) objects consist of two [Num](https://docs.perl6.org/type/Num) objects representing the [real](https://docs.perl6.org/routine/re) and [imaginary](https://docs.perl6.org/routine/im) portions of the complex number.
...
To create a [Complex](https://docs.perl6.org/type/Complex), you can use the [postfix `i` operator](https://docs.perl6.org/routine/i) on any other non-complex number, optionally setting the real part with addition. To use the `i` operator on `NaN` or `Inf` literals, separate it from them with a backslash.
...
The types that do the [Rational](https://docs.perl6.org/type/Rational) role offer high-precision and arbitrary-precision decimal numbers. Since the higher the precision the larger the performance penalty, the [Rational](https://docs.perl6.org/type/Rational) types come in two flavors: [Rat](https://docs.perl6.org/type/Rat) and [FatRat](https://docs.perl6.org/type/FatRat). The [Rat](https://docs.perl6.org/type/Rat) is the most often-used variant that degrades into a [Num](https://docs.perl6.org/type/Num) in most cases, when it can no longer hold all of the requested precision. The [FatRat](https://docs.perl6.org/type/FatRat) is the arbitrary-precision variant that keeps growing to provide all of the requested precision.
...
Lastly, any Unicode character with property `No` that represents a fractional number can be used as a [Rat](https://docs.perl6.org/type/Rat) literal:
...
如果产生 [Rat](https://docs.perl6.org/type/Rat) 答案的*数学运算*会产生分母大于 64 位的 [Rat](https://docs.perl6.org/type/Rat)，则该操作将返回 [Num](https://docs.perl6.org/type/Num) 对象。当*构建*一个[Rat](https://docs.perl6.org/type/Rat)（即，当它不是一些数学表达式的结果）时，但是，可以使用更大的分母：
...
The last [Rational](https://docs.perl6.org/type/Rational) type—[FatRat](https://docs.perl6.org/type/FatRat)—keeps all of the precision you ask of it, storing the numerator and denominator as two [Int](https://docs.perl6.org/type/Int)objects. A [FatRat](https://docs.perl6.org/type/FatRat) is more infectious than a [Rat](https://docs.perl6.org/type/Rat), so many math operations with a [FatRat](https://docs.perl6.org/type/FatRat) will produce another [FatRat](https://docs.perl6.org/type/FatRat), preserving all of the available precision. Where a [Rat](https://docs.perl6.org/type/Rat) degrades to a [Num](https://docs.perl6.org/type/Num), math with a [FatRat](https://docs.perl6.org/type/FatRat) keeps chugging along:
...
Perl 6 遵循 [IEEE 754-2008 浮点运算标准](https://en.wikipedia.org/wiki/IEEE_754)，但由于历史原因，6.c 和 6.d 语言版本不完全符合。[Num](https://docs.perl6.org/type/Num) 被零除产生 [Failure](https://docs.perl6.org/type/Failure)，而[复数](https://docs.perl6.org/type/Complex)被零除产生 `NaN` 部件, 无论分子是什么。

Perl 6 follows [IEEE 754-2008 Standard for Floating-Point Arithmetic](https://en.wikipedia.org/wiki/IEEE_754), but for historical reasons 6.c and 6.d language versions do not comply fully. [Num](https://docs.perl6.org/type/Num) division by zero produces a [Failure](https://docs.perl6.org/type/Failure), while [Complex](https://docs.perl6.org/type/Complex) division by zero produces `NaN`components, regardless of what the numerator is.
...
As of 6.e language, both [Num](https://docs.perl6.org/type/Num) and [Complex](https://docs.perl6.org/type/Complex) division by zero will produce a -[Inf](https://docs.perl6.org/type/Num#Inf), `+Inf`, or [NaN](https://docs.perl6.org/type/Num#NaN) depending on whether the numerator was negative, positive, or zero, respectively (for [Complex](https://docs.perl6.org/type/Complex) the real and imaginary components are [Num](https://docs.perl6.org/type/Num) and are considered separately).
...
Division of [Int](https://docs.perl6.org/type/Int) numerics produces a [Rat](https://docs.perl6.org/type/Rat) object (or a [Num](https://docs.perl6.org/type/Num), if after reduction the denominator is larger than 64-bits, which isn't the case when you're dividing by zero). This means such division never produces an [Exception](https://docs.perl6.org/type/Exception) or a [Failure](https://docs.perl6.org/type/Failure). The result is a Zero-Denominator Rational, which can be explosive.
...
[Allomorphs](https://docs.perl6.org/language/glossary#index-entry-Allomorph) are subclasses of two types that can behave as either of them. For example, the allomorph [IntStr](https://docs.perl6.org/type/IntStr) is the subclass of [Int](https://docs.perl6.org/type/Int) and [Str](https://docs.perl6.org/type/Str) types and will be accepted by any type constraint that requires an [Int](https://docs.perl6.org/type/Int) or [Str](https://docs.perl6.org/type/Str) object.
...
A couple of constructs above have a space after the opening angle bracket. That space isn't accidental. Numerics that are often written using an operator, such as `1/2` ([Rat](https://docs.perl6.org/type/Rat), division operator) and `1+2i` ([Complex](https://docs.perl6.org/type/Complex), addition) can be written as a literal that doesn't involve the use of an operator: angle brackets *without* any spaces between the angle brackets and the characters inside. By adding spaces within the angle brackets, we tell the compiler that not only we want a [Rat](https://docs.perl6.org/type/Rat) or [Complex](https://docs.perl6.org/type/Complex)literal, but we also want it to be an allomorph: the [RatStr](https://docs.perl6.org/type/RatStr) or [ComplexStr](https://docs.perl6.org/type/ComplexStr), in this case.
...
当然，这也适用于参数[强制类型转换器](https://docs.perl6.org/type/Signature#Coercion_type)：

This, of course, also applies to parameter [coercers](https://docs.perl6.org/type/Signature#Coercion_type):
...
A handy way to coerce a whole list of allomorphs is by applying the [hyper operator](https://docs.perl6.org/language/operators#Hyper_operators) to the appropriate prefix:
...
其次，如果例程是一个 `only`-也就是说，它不是一个接受非原生类型的 [`multi`](https://docs.perl6.org/language/functions#Multi-dispatch) 例程，但是在调用期间给了例程一个原生类型，反之亦然，那么参数将被自动装箱或自动取消装箱以使可以被调用。如果给定的参数太大而无法放入原生参数，则会抛出异常：
...
When it comes to [`multi`](https://docs.perl6.org/language/functions#Multi-dispatch) routines, native arguments will always be auto-boxed if no native candidates are available to take them:
...
However, this rule is waived if a call is being made where one of the arguments is a native type and another one is a [numeric literal](https://docs.perl6.org/language/syntax#Number_literals):
...
该语言提供了保证以原子方式执行的[一些操作](https://docs.perl6.org/type/atomicint)，即安全地由多个线程执行而无需锁定而没有数据争用的风险。

The language offers [some operations](https://docs.perl6.org/type/atomicint) that are guaranteed to be performed atomically, i.e. safe to be executed by multiple threads without the need for locking with no risk of data races.

对于此类操作，需要 [atomicint](https://docs.perl6.org/type/atomicint) 原生类型。此类型与普通原生 [int](https://docs.perl6.org/type/int) 类似，不同之处在于它的大小使得可以对其执行 CPU 提供的原子操作。在 32 位 CPU 上，它通常是 32 位大小，而在 64 位 CPU 上，它通常是 64 位大小。

For such operations, the [atomicint](https://docs.perl6.org/type/atomicint) native type is required. This type is similar to a plain native [int](https://docs.perl6.org/type/int), except it is sized such that CPU-provided atomic operations can be performed upon it. On a 32-bit CPU it will typically be 32 bits in size, and on an a 64-bit CPU it will typically be 64 bits in size.
...
当一些数学运算中涉及两个不同类型的数字时，数字“传染性”决定了结果类型。如果结果是该类型而不是其他操作数的类型，则认为类型比其他类型更具传染性。例如，[Num](https://docs.perl6.org/type/Num) 类型比 [Int](https://docs.perl6.org/type/Int) 更具传染性，因此我们可以期望 `42e0 + 42` 产生 [Num](https://docs.perl6.org/type/Num) 作为结果。

Numeric "infectiousness" dictates the resultant type when two numerics of different types are involved in some mathematical operations. A type is said to be more infectious than the other type if the result is of that type rather than the type of the other operand. For example, [Num](https://docs.perl6.org/type/Num) type is more infectious than an [Int](https://docs.perl6.org/type/Int), thus we can expect `42e0 + 42` to produce a [Num](https://docs.perl6.org/type/Num) as the result.
