...
Applies the [is dynamic](https://docs.perl6.org/type/Variable#trait_is_dynamic) trait to variables in the pragma's lexical scope. The effect can be restricted to a subset of variables by listing their names as arguments. By default applies to *all* variables.
...
Allows use of [experimental features](https://docs.perl6.org/language/experimental)



## fatal

A lexical pragma that makes [Failures](https://docs.perl6.org/type/Failure) returned from routines fatal. For example, prefix `+` on a [Str](https://docs.perl6.org/type/Str) coerces it to [Numeric](https://docs.perl6.org/type/Numeric), but will return a [Failure](https://docs.perl6.org/type/Failure) if the string contains non-numeric characters. Saving that [Failure](https://docs.perl6.org/type/Failure) in a variable prevents it from being sunk, and so the first code block below reaches the `say $x.^name;` line and prints `Failure` in output.

In the second block, the `use fatal` pragma is enabled, so the `say` line is never reached because the [Exception](https://docs.perl6.org/type/Exception) contained in the [Failure](https://docs.perl6.org/type/Failure) returned from prefix `+` gets thrown and the `CATCH` block gets run, printing the `Caught...` line. Note that both blocks are the same program and `use fatal` only affects the lexical block it was used in:
...
Inside [`try` blocks](https://docs.perl6.org/language/exceptions#index-entry-try_blocks-try), the `fatal` pragma is enabled by default, and you can *disable* it with `no fatal`:
...
This pragma adds subdirectories to the library search path so that the interpreter can [find the modules](https://docs.perl6.org/language/modules#Finding_modules).

```Perl6
use lib <lib /opt/lib /usr/local/lib>;
```

This will search the directories passed in a list. Please check [the modules documentation](https://docs.perl6.org/language/modules#use) for more examples.



## newline

Set the value of the [$?NL](https://docs.perl6.org/language/variables#Compile-time_variables) constant in the scope it is called. Possible values are `:lf` (which is the default, indicating Line Feed), `:crlf` (indicating Carriage Return, Line Feed) and `:cr` (indicating Carriage Return).
...
[Re-dispatching](https://docs.perl6.org/language/functions#Re-dispatching), [inlining](https://docs.perl6.org/language/functions#index-entry-use_soft_(pragma))
