原文： https://docs.raku.org/language/io-guide
...
虽然 [IO::Socket](https://docs.raku.org/type/IO::Socket) 及其子类也与输入和输出有关，但本指南不介绍它们。
...
这意味着，不管你是如何创建的，[IO::Path](https://docs.raku.org/type/IO::Path) 对象技术上总是引用绝对路径。这就是它的 [`.absolute`](https://docs.raku.org/routine/absolute) 和 [`.relative`](https://docs.raku.org/routine/relative) 方法返回 [Str](https://docs.raku.org/type/Str) 对象的原因，它们是正确的路径字符串化方法。

This means that regardless of how you made one, an [IO::Path](https://docs.raku.org/type/IO::Path) object technically always refers to an absolute path. This is why its [`.absolute`](https://docs.raku.org/routine/absolute) and [`.relative`](https://docs.raku.org/routine/relative) methods return [Str](https://docs.raku.org/type/Str) objects and they are the correct way to stringify a path.

但是，不要急于将任何东西字符串化。将文件路径作为 [IO::Path](https://docs.raku.org/type/IO::Path) 对象传递。所有在文件路径上操作的例程都可以处理它们，因此不需要转换它们。
...
但是，这就是你所需要的所有代码。默认情况下，字符串将被编码为 `utf-8` 编码，错误通过 [Failure](https://docs.raku.org/type/Failure) 机制处理：这些是可以使用常规条件处理的异常。在这种情况下，我们会让所有潜在的 [Failure](https://docs.raku.org/type/Failure) 在调用后都被下沉，因此它们包含的任何 [Exceptions](https://docs.raku.org/type/Exception) 都将被抛出。
...
If you wanted to add more content to the file we made in previous section, you could note the [`spurt` documentation](https://docs.raku.org/routine/spurt) mentions `:append` argument. However, for finer control, let's get ourselves an [IO::Handle](https://docs.raku.org/type/IO::Handle) to work with:
...
[`.open`](https://docs.raku.org/routine/open) 方法调用打开我们的 [IO::Path](https://docs.raku.org/type/IO::Path) 并返回一个 [IO::Handle](https://docs.raku.org/type/IO::Handle)。我们传递了 `:a` 作为参数，以指示我们要以追加模式打开文件进行写入。

The [`.open`](https://docs.raku.org/routine/open) method call opens our [IO::Path](https://docs.raku.org/type/IO::Path) and returns an [IO::Handle](https://docs.raku.org/type/IO::Handle). We passed `:a` as argument, to indicate we want to open the file for writing in append mode.

在接下来的两行代码中，我们在该 [IO::Handle](https://docs.raku.org/type/IO::Handle) 方法上使用了常用的 [`.print`](https://docs.raku.org/routine/print) 来打印包含 11 个文本碎片的行（`'I count: '` 字符串和 10 个数字）。请注意，再一次，[Failure](https://docs.raku.org/type/Failure) 机制负责我们所有的错误检查。如果 [`.open`](https://docs.raku.org/routine/open) 失败，它将返回一个 [Failure](https://docs.raku.org/type/Failure)，当我们试图调用方法 [`.print`]（https://docs.raku.org/routine/print）时，它将抛出这个异常。

In the next two lines of code, we use the usual [`.print`](https://docs.raku.org/routine/print) method on that [IO::Handle](https://docs.raku.org/type/IO::Handle) to print a line with 11 pieces of text (the `'I count: '` string and 10 numbers). Note that, once again, [Failure](https://docs.raku.org/type/Failure) mechanism takes care of all the error checking for us. If the [`.open`](https://docs.raku.org/routine/open) fails, it returns a [Failure](https://docs.raku.org/type/Failure), which will throw when we attempt to call method [`.print`](https://docs.raku.org/routine/print) on it.

最后，我们通过调用 [`.close`](https://docs.raku.org/routine/close) 方法来关闭 [IO::Handle](https://docs.raku.org/type/IO::Handle)。做这件事非常重要，尤其是在大型程序或处理大量文件的程序中，因为许多系统对一个程序可以同时打开的文件数量有限制。如果不关闭句柄，最终将达到该限制，则 [`.open`](https://docs.raku.org/routine/open) 调用将失败。请注意，与其他一些语言不同，Raku 不使用引用计数，因此在离开文件句柄定义的作用域时，文件句柄**不会关闭**。它们只有在被垃圾收集时才会被关闭，如果不关闭句柄，可能会导致程序在打开的句柄有机会被垃圾收集*之前*达到文件句柄数量限制。
...
[`.slurp`](https://docs.raku.org/routine/slurp) 方法读取文件的全部内容，并将其作为单个 [Str](https://docs.raku.org/type/Str) 对象返回，或者作为 [Buf](https://docs.raku.org/type/Buf) 对象返回，如果需要返回二进制，则通过指定 `:bin` 命名参数。

The [`.slurp`](https://docs.raku.org/routine/slurp) method reads entire contents of the file and returns them as a single [Str](https://docs.raku.org/type/Str) object, or as a [Buf](https://docs.raku.org/type/Buf) object, if binary mode was requested, by specifying `:bin` named argument.

因为 [slurp](https://docs.raku.org/routine/slurp) 将整个文件加载到内存中，所以不适合处理大型文件。

Since [slurping](https://docs.raku.org/routine/slurp) loads the entire file into memory, it's not ideal for working with huge files.

[IO::Path](https://docs.raku.org/type/IO::Path) 类型提供了另外两种方便的方法： [`.words`](https://docs.raku.org/type/IO::Path#method_words) 和 [`.lines`](https://docs.raku.org/type/IO::Path#method_lines)，它们可以轻松地将文件分块读取并返回 [Seq](https://docs.raku.org/type/Seq) 对象，默认情况下不保留已消耗值的对象。

The [IO::Path](https://docs.raku.org/type/IO::Path) type offers two other handy methods: [`.words`](https://docs.raku.org/type/IO::Path#method_words) and [`.lines`](https://docs.raku.org/type/IO::Path#method_lines) that lazily read the file in smaller chunks and return [Seq](https://docs.raku.org/type/Seq) objects that (by default) don't keep already-consumed values around.
...
请注意，我们通过将限制参数传递给 [`.words`](https://docs.raku.org/type/IO::Path#method_words)，而不是使用[列表索引操作](https://docs.raku.org/language/operators#index-entry-array_indexing_operator-array_subscript_operator-array_indexing_operator)。原因是底层仍有一个文件句柄在使用，在你完全消耗掉返回的 [Seq](https://docs.raku.org/type/Seq) 之前，该句柄将保持打开状态。如果没有任何内容引用 [Seq](https://docs.raku.org/type/Seq)，那么在垃圾收集运行期间，最终会关闭句柄，但是在处理大量文件的大型程序中，最好确保立即关闭所有句柄。因此，你应该始终确保来自 [IO::Path](https://docs.raku.org/type/IO::Path) 的 [`.words`](https://docs.raku.org/type/IO::Path#method_words) 和 [`.lines`](https://docs.raku.org/type/IO::Path#method_lines) 方法是[完全具体化的](https://docs.raku.org/language/glossary#index-entry-Reify)；这里的 limit 参数可以帮助你实现这一点。
...
Of course, you can read from files using the [IO::Handle](https://docs.raku.org/type/IO::Handle) type, which gives you a lot finer control over what you're doing:
...
You may have heard of [`$*SPEC`](https://docs.raku.org/language/variables#Dynamic_variables) and seen some code or books show its usage for splitting and joining path fragments. Some of the routine names it provides may even look familiar to what you've used in other languages.

但是，除非你正在编写自己的 IO 框架，否则几乎不需要直接使用 [`$*SPEC`](https://docs.raku.org/language/variables#Dynamic_variables)。[`$*SPEC`](https://docs.raku.org/language/variables#Dynamic_variables) 提供了底层的东西，它的使用不仅会使你的代码难以阅读，还可能带来安全问题（如 null 字符）！
...
[`IO::Path`](https://docs.raku.org/type/IO::Path) 类型是 Raku 世界的老黄牛。它满足了所有文件路径操作的需要，并提供了快捷例程，使你可以避免处理文件句柄。使用它而不是 [`$*SPEC`](https://docs.raku.org/language/variables#Dynamic_variables)。

The [`IO::Path`](https://docs.raku.org/type/IO::Path) type is the workhorse of Raku world. It caters to all the path manipulation needs as well as provides shortcut routines that let you avoid dealing with filehandles. Use that instead of the [`$*SPEC`](https://docs.raku.org/language/variables#Dynamic_variables) stuff.

提示：你可以使用 `/` 连接文件路径部件，并将它们馈送给 [`IO::Path`](https://docs.raku.org/type/IO::Path) 的例程；不管操作系统如何，它们仍然可以对它们执行正确的操作。

Tip: you can join path parts with `/` and feed them to [`IO::Path`](https://docs.raku.org/type/IO::Path)'s routines; they'll still do The Right Thing™ with them, regardless of the operating system.
...
但是，可以将其用于 [IO::Path](https://docs.raku.org/type/IO::Path) 未提供的内容。例如，[`.devnull`方法](https://docs.raku.org/routine/devnull)：

However, it's fine to use it for things not otherwise provided by [IO::Path](https://docs.raku.org/type/IO::Path). For example, the [`.devnull` method](https://docs.raku.org/routine/devnull):
...
Don't use the `.Str` method to stringify [`IO::Path`](https://docs.raku.org/type/IO::Path) objects, unless you just want to display them somewhere for information purposes or something. The `.Str` method returns whatever basic path string the [`IO::Path`](https://docs.raku.org/type/IO::Path) was instantiated with. It doesn't consider the value of the [`$.CWD` attribute](https://docs.raku.org/type/IO::Path#attribute_CWD). For example, this code is broken:
...
[`chdir`](https://docs.raku.org/routine/chdir) 调用更改了当前目录的值，但我们创建的 `$path` 是相对于更改前的目录的。

The [`chdir`](https://docs.raku.org/routine/chdir) call changed the value of the current directory, but the `$path` we created is relative to the directory before that change.

但是，[`IO::Path`](https://docs.raku.org/type/IO::Path) 对象*的确*知道它与哪个目录相关。我们只需要使用 [`.absolute`](https://docs.raku.org/routine/absolute) 或 [`.relative`](https://docs.raku.org/routine/relative) 对对象进行字符串化。两个例程都返回一个 [`Str`](https://docs.raku.org/type/Str) 对象；它们只在结果是绝对路径还是相对路径上有所不同。因此，我们可以这样修复代码：

However, the [`IO::Path`](https://docs.raku.org/type/IO::Path) object *does* know what directory it's relative to. We just need to use [`.absolute`](https://docs.raku.org/routine/absolute) or [`.relative`](https://docs.raku.org/routine/relative) to stringify the object. Both routines return a [`Str`](https://docs.raku.org/type/Str) object; they only differ in whether the result is an absolute or relative path. So, we can fix our code like this:
...
`my $*CWD` 使得 [`$*CWD`](https://docs.raku.org/language/variables#Dynamic_variables) 未定义。[`.IO`](https://docs.raku.org/routine/IO) 强制转换程序继续执行，并将其创建的文件路径的 [`$.CWD` 属性](https://docs.raku.org/type/IO::Path#attribute_CWD) 设置为未定义的 `$*CWD` 的字符串版本；一个空字符串。

The `my $*CWD` made [`$*CWD`](https://docs.raku.org/language/variables#Dynamic_variables) undefined. The [`.IO`](https://docs.raku.org/routine/IO) coercer then goes ahead and sets the [`$.CWD` attribute](https://docs.raku.org/type/IO::Path#attribute_CWD) of the path it's creating to the stringified version of the undefined `$*CWD`; an empty string.

执行此操作的正确方法是使用 [`temp`](https://docs.raku.org/routine/temp)，而不是使用 `my`。它将把更改的效果本地化为 [`$*CWD`](https://docs.raku.org/language/variables#Dynamic_variables)，就像 `my` 一样，但它不会使其未定义，因此 [`.IO`](https://docs.raku.org/routine/IO) 强制转换程序仍将获得正确的旧值：

The correct way to perform this operation is use [`temp`](https://docs.raku.org/routine/temp) instead of `my`. It'll localize the effect of changes to [`$*CWD`](https://docs.raku.org/language/variables#Dynamic_variables), just like `my` would, but it won't make it undefined, so the [`.IO`](https://docs.raku.org/routine/IO) coercer will still get the correct old value:

```Raku
temp $*CWD = "foo".IO;
```

更好的是，如果你想在本地化的 [`$*CWD`](https://docs.raku.org/language/variables#Dynamic_variables) 中执行一些代码，请使用 [`indir` 例程](https://docs.raku.org/routine/indir)。

Better yet, if you want to perform some code in a localized [`$*CWD`](https://docs.raku.org/language/variables#Dynamic_variables), use the [`indir` routine](https://docs.raku.org/routine/indir) for that purpose.