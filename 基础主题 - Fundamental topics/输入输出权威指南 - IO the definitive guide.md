原文： https://docs.perl6.org/language/io-guide
...
大多数常见的 IO 工作都是由 [IO::Path](https://docs.perl6.org/type/IO::Path) 类型完成的。如果你想以某种形式或形状读取或写入文件，这就是你想要的类。它抽象掉了文件句柄（或“文件描述符”）的细节，因此你几乎不必考虑它们。
...
最后，你有 [IO::CatHandle](https://docs.perl6.org/type/IO::CatHandle)，以及 [IO::Spec](https://docs.perl6.org/type/IO::Spec) 及其子类，你很少直接使用它们。这些类为你提供了高级功能，例如将多个文件作为一个句柄进行操作，或者提供低级路径操作。
...
与所有这些类一起，Perl 6 提供了几个子例程，可以让你间接地使用这些类。如果你喜欢函数式编程风格或 Perl 6 一行程序，这些都很有用。
...
虽然 [IO::Socket](https://docs.perl6.org/type/IO::Socket) 及其子类也与输入和输出有关，但本指南不介绍它们。
...
要将路径表示为文件或目录，请使用 [IO::Path](https://docs.perl6.org/type/IO::Path) 类型。获取该类型对象的最简单方法是通过对其调用 [`.IO`](https://docs.perl6.org/routine/IO) 方法来强制转换 [Str](https://docs.perl6.org/type/Str) 类型：
...
这意味着，不管你是如何创建的，[IO::Path](https://docs.perl6.org/type/IO::Path) 对象技术上总是引用绝对路径。这就是它的 [`.absolute`](https://docs.perl6.org/routine/absolute) 和 [`.relative`](https://docs.perl6.org/routine/relative) 方法返回 [Str](https://docs.perl6.org/type/Str) 对象的原因，它们是正确的路径字符串化方法。
...
让我们制作一些文件，并从中写入和读取数据！[`spurt`](https://docs.perl6.org/routine/spurt) 和 [`slurp`](https://docs.perl6.org/routine/slurp) 例程在一个块中写入和读取数据。除非你正在处理非常大的文件，这些文件很难同时完全存储在内存中，否则这两个例程都是为你准备的。
...
上面的代码在当前目录中创建一个名为 `my-file.txt` 的文件，然后写入文本 `I ♥ Perl!`。如果 Perl 6 是你的第一语言，请庆祝你的成就！尝试打开用其他程序创建的文件，以验证用程序编写的内容。如果你已经知道其他语言，你可能会想知道本指南是否遗漏了处理编码或错误条件之类的内容。
...
[`.open`](https://docs.perl6.org/routine/open) 方法调用打开我们的 [IO::Path](https://docs.perl6.org/type/IO::Path) 并返回一个 [IO::Handle](https://docs.perl6.org/type/IO::Handle)。我们传递了 `:a` 作为参数，以指示我们要以追加模式打开文件进行写入。
...
在前面的部分中，我们已经看到在 Perl 6 中，将内容写入文件只是一行代码。从他们那里读，同样容易：
...
[`.slurp`](https://docs.perl6.org/routine/slurp) 方法读取文件的全部内容，并将其作为单个 [Str](https://docs.perl6.org/type/Str) 对象返回，或者作为 [Buf](https://docs.perl6.org/type/Buf) 对象返回，如果需要返回二进制，则通过指定 `:bin` 命名参数。
...
[IO::Path](https://docs.perl6.org/type/IO::Path) 类型提供了另外两种方便的方法： [`.words`](https://docs.perl6.org/type/IO::Path#method_words) 和 [`.lines`](https://docs.perl6.org/type/IO::Path#method_lines)，它们可以轻松地将文件分块读取并返回 [Seq](https://docs.perl6.org/type/Seq) 对象，默认情况下不保留已消耗值的对象。
...
当然，你可以使用 [IO::Handle](https://docs.perl6.org/type/IO::Handle) 类型从文件中读取，这样你可以更好地控制正在执行的操作：
...
你可能听说过 [`$*SPEC`](https://docs.perl6.org/language/variables#Dynamic_variables) 并看到一些代码或书籍展示了它对分割和连接路径片段的用法。它提供的一些例程名称甚至可能看起来与你在其他语言中使用的名称很类似。
...
但是，可以将其用于 [IO::Path](https://docs.perl6.org/type/IO::Path) 未提供的内容。例如，[`.devnull`方法](https://docs.perl6.org/routine/devnull)：
...
[`chdir`](https://docs.perl6.org/routine/chdir) 调用更改了当前目录的值，但我们创建的 `$path` 是相对于更改前的目录的。
...
但是，[`IO::Path`](https://docs.perl6.org/type/IO::Path) 对象*的确*知道它与哪个目录相关。我们只需要使用 [`.absolute`](https://docs.perl6.org/routine/absolute) 或 [`.relative`](https://docs.perl6.org/routine/relative) 对对象进行字符串化。两个例程都返回一个 [`Str`](https://docs.perl6.org/type/Str) 对象；它们只在结果是绝对路径还是相对路径上有所不同。因此，我们可以这样修复代码：
...
更好的是，如果你想在本地化的 [`$*CWD`](https://docs.perl6.org/language/variables#Dynamic_variables) 中执行一些代码，请使用 [`indir` 例程](https://docs.perl6.org/routine/indir)。
